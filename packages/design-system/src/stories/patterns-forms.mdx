...
- [`DsButtonV`](?path=/docs/design-system-button--docs)
...
- Always disable while submitting, and surface the in-flight state through
  [`DsButton`](?path=/docs/design-system-button--docs)'s `loading` prop so the button renders its
...
<Source
	dark
	language="tsx"
	code={`<DsButton type="submit" disabled={!isDirty || isSubmitting} loading={isSubmitting}>
    Submit
</DsButton>`}
/>
...
<Source
	dark
	language="tsx"
	code={`<DsButton type="submit" disabled={isSubmitting} loading={isSubmitting}>
    Submit
</DsButton>`}
/>
