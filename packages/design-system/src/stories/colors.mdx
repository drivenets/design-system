...
<DsTypography variant="body-md-reg" className={styles.pageDescription}>
	Reference CSS variables instead of hard-coding hex values. Tokens are organized in three layers:
	<b> primitive</b> (raw palette), <b>alias</b> (intent-based naming), and <b>semantic</b> (role-based usage).
	Click any swatch to copy the token. Source of truth: <code>src/styles/_root_new.scss</code>.
</DsTypography>
...
<DsTypography variant="body-md-reg" className={styles.layerDescription}>
	The raw palette. Prefer the alias or semantic layers below — only reach for primitive tokens when no
	suitable abstraction exists.
</DsTypography>

<div className={`${styles.container} ${styles.layer}`}>
...
<DsTypography variant="body-md-reg" className={styles.layerDescription}>
	Intent-based aliases that point at the primitive palette. Prefer these over primitives when the semantic
	layer doesn't cover the use case.
</DsTypography>
...
<DsTypography variant="body-md-reg" className={styles.layerDescription}>
	...
	Always prefer these when composing components.
</DsTypography>
...
<SwatchGroup
	...
	tones={[
		...
		{ label: 'bg-netural', variable: 'status-bg-netural' },
