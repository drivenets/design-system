...
export const getCssVariable = (name) => {
	...
	if (varRef) {
		value = getComputedStyle(document.documentElement).getPropertyValue(`--${varRef[1]}`).trim() || value;
	}
	return /^#([0-9a-f]{3,8})$/i.test(value) ? value.toUpperCase() : value;
};

export const isHexTone = (value) => /^#([0-9a-f]{3,8})$/i.test(value);
...
export const Swatch = ({ variable, label }) => {
    ...
    const showToneCode = isHexTone(value);
    ...
    return (
    	<div className={styles.swatchCol}>
    		...
    		>
    			{showToneCode && <span className={styles.toneCode}>{value}</span>}
...
export const GradientCard = ({ variable, title }) => {
	const [copied, setCopied] = useState(false);
	const handleCopy = async () => {
		await navigator.clipboard.writeText(`var(--${variable})`);
		setCopied(true);
    ...
    return (
    	<div className={`${styles.gradientCard} sb-unstyled`}>
    		<button
    			type="button"
    			...
    			onClick={handleCopy}
    			title={`Click to copy var(--${variable})`}
    			aria-label={`${title} var(--${variable})`}
    		>
    			{copied && <span className={styles.copiedTooltip}>Copied</span>}
    		</button>
    		<div className={styles.gradientMeta}>
    			<DsTypography variant="heading4">{title}</DsTypography>
    			<code className={styles.gradientCode}>background: var(--{variable});</code>
    		</div>
    	</div>
    );
...
<SwatchGroup
	...
	tones={[
		...
		{ label: 'light-active', variable: 'background-light-active' },
		{ label: 'light-disabled', variable: 'background-light-disabled' },
		{ label: 'light-hover', variable: 'background-light-hover' },
		...
		{ label: 'light-subtle', variable: 'background-light-subtle' },
...
<SwatchGroup
	label="Device"
	tones={[
		{ label: 'green', variable: 'device-green' },
		{ label: 'green-20', variable: 'device-green-20' },
		{ label: 'light-blue', variable: 'device-light-blue' },
		{ label: 'light-blue-20', variable: 'device-light-blue-20' },
		{ label: 'orange', variable: 'device-orange' },
		{ label: 'orange-20', variable: 'device-orange-20' },
		{ label: 'purple', variable: 'device-purple' },
		{ label: 'purple-20', variable: 'device-purple-20' },
	]}
/>
...
<SwatchGroup
	label="DAP icons"
	tones={[
		{ label: '0', variable: 'dap-icons-0' },
		{ label: '50', variable: 'dap-icons-50' },
		{ label: '75', variable: 'dap-icons-75' },
		{ label: '100', variable: 'dap-icons-100' },
		{ label: '200', variable: 'dap-icons-200' },
		{ label: '300', variable: 'dap-icons-300' },
		{ label: '400', variable: 'dap-icons-400' },
		{ label: '500', variable: 'dap-icons-500' },
		{ label: '600', variable: 'dap-icons-600' },
		{ label: '700', variable: 'dap-icons-700' },
		{ label: '800', variable: 'dap-icons-800' },
		{ label: '900', variable: 'dap-icons-900' },
		{ label: '950', variable: 'dap-icons-950' },
		{ label: 'grey', variable: 'dap-icons-grey' },
	]}
/>
...
Hand-authored in `_gradients.scss`. The design system ships **only**
shared Brand gradient recipes used by DS. Prefer these tokens when the
surface matches their Figma source, do not rebuild the same recipe from stop tokens.

| Token                          | Use                               |
| ------------------------------ | --------------------------------- |
| `--blue-radial-grad`           | General Brand Refresh blue radial |
| `--blue-grad-workspace-header` | `DsWorkspaceLayout.Header` only   |

Stop tokens `--blue-grad-1`, `--blue-grad-2`, etc. exist for composing related gradients.

### App-owned gradients (consumers)

Product apps **define their own gradients in the consuming application** (app CSS / theme),
not in `@drivenets/design-system`.

1. Add a CSS custom property in the app (e.g. `--my-feature-header-grad`).
2. Prefer stop / semantic color tokens (`--blue-grad-*`, `--primary-*`, `--color-dap-*`)
   for the color stops so Brand Refresh hues stay consistent.
3. Keep geometry (ellipse size, position, alpha) local to the app — that is product chrome,
   not a design-system API.

<div className={styles.layer}>

<GradientCard variable="blue-radial-grad" title="Blue-grad" />

<GradientCard variable="blue-grad-workspace-header" title="Blue-grad workspace header" />

<SwatchGroup
	label="Stops"
	tones={[
		{ label: 'grad-1', variable: 'blue-grad-1' },
		{ label: 'grad-2', variable: 'blue-grad-2' },
	]}
/>
