...
<div>
  <div className={styles.sectionHeader}>
    <DsIcon icon="smart_toy" size="medium" />
    <DsTypography variant="heading2">Design System MCP</DsTypography>
  </div>

<DsTypography variant="body-md-reg" asChild className={styles.sectionDescription}>
	<span>
		Give your AI assistant (Cursor, Claude Code, Codex, …) access to live component docs, props, and code
		snippets straight from this Storybook. The command below adds the MCP server to every IDE on your machine.
	</span>
</DsTypography>

  <div className={styles.steps}>
    <div className={styles.step}>
      <Source dark code="npx add-mcp@latest -g @drivenets/design-system-mcp" language="bash" />
      <DsTypography variant="body-sm-reg" asChild className={styles.docsLink}>
        <a
          href="https://github.com/drivenets/design-system/blob/main/packages/mcp/README.md"
          target="_blank"
          rel="noopener noreferrer"
        >
          Manual config & per-IDE details →
        </a>
      </DsTypography>
    </div>
  </div>
...
<div>
  <div className={styles.sectionHeader}>
    <DsIcon icon="bolt" size="medium" />
    <DsTypography variant="heading2">Performance Panel</DsTypography>
  </div>

<DsTypography variant="body-md-reg" asChild className={styles.sectionDescription}>
	<span>
		Every story has a live ⚡ Performance tab at the bottom of the Canvas. It tracks frame timing (FPS), input
		responsiveness (INP), long tasks, layout shift (CLS), React render metrics (mount, slow updates, render
		cascades), and memory in real time as you interact — so you can catch regressions while developing, before
		they ship. Open a story in Canvas view and interact with it to populate the metrics.
	</span>
</DsTypography>

  <div className={styles.steps}>
    <div className={styles.step}>
      <DsTypography variant="heading4">Silence it for a component</DsTypography>
      <DsTypography variant="body-sm-reg" className={styles.stepHint}>
        Add this to a story meta if the panel is noisy or not useful for that component:
      </DsTypography>
      <Source dark code="parameters: { performancePanel: { disable: true } }" language="tsx" />
      <DsTypography variant="body-sm-reg" asChild className={styles.docsLink}>
        <a
          href="https://github.github.com/storybook-addon-performance-panel/"
          target="_blank"
          rel="noopener noreferrer"
        >
          Metrics reference & how to read them →
        </a>
      </DsTypography>
    </div>
  </div>
