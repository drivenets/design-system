...
Maps old token names to their current replacements.

> **The old tokens no longer exist in the codebase.** This page is kept as a
> lookup reference because Figma designs and older documentation may still use the old names — use it
> to find the token you should apply instead.
...
| Removed | Old token with no new equivalent                                                                       | The **New Name** column holds a literal CSS value to use verbatim (any `var(--token)` is unwrapped so the resulting CSS stays valid) |
