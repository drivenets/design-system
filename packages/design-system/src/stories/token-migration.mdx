...
### Removed (kept under `_kept-tokens.scss`, pending relocation to components)

These tokens have no equivalent in `_root_new.scss` and are intentionally
preserved. Their definitions live in `packages/design-system/src/styles/_kept-tokens.scss`
and will be moved next to the components that own them
(`--letter-spacing--1percent` → typography mixins, `--icon-width-*` → DsIcon,
`--ds-table-row-*` / `--ds-table-header-height` → DsTable).
...
| Old Name                 | New Name     | Status  |
| ------------------------ | ------------ | ------- |
| `--base-unit`            | `--3xs`      | Changed |
| `--spacing-1`            | `--3xs`      | Changed |
| `--spacing-2`            | `--xs`       | Changed |
| `--spacing-3`            | `--sm`       | Changed |
| `--spacing-4`            | `--standard` | Changed |
| `--spacing-5`            | `--md`       | Changed |
| `--spacing-6`            | `--lg`       | Changed |
| `--spacing-8`            | `--xl`       | Changed |
| `--spacing-10`           | —            | Removed |
| `--spacing-12`           | `--2xl`      | Changed |
| `--spacing-16`           | `--3xl`      | Changed |
| `--spacing-20`           | `--4xl`      | Changed |
| `--spacing-24`           | `--5xl`      | Changed |
| `--spacing-base`         | `--standard` | Changed |
| `--exo-spacing-standard` | `--standard` | Changed |
| `--exo-spacing-small`    | `--sm`       | Changed |
| `--exo-spacing-x-small`  | `--xs`       | Changed |
| `--exo-spacing-2x-small` | `--3xs`      | Changed |
