...
// source=https://github.com/drivenets/design-system/tree/main/packages/design-system/src/components/ds-button
// component=DsButton
...
export default {
	example: iconOnly
		? figma.code`<DsButton variant="${variant}" color="${color}" size="${size}" ${disabled ? 'disabled' : ''}${iconProp} aria-label="${label}" />`
		: showIcon && icon
			? figma.code`<DsButton variant="${variant}" color="${color}" size="${size}" ${disabled ? 'disabled' : ''} icon="${icon}">${label}</DsButton>`
			: figma.code`<DsButton variant="${variant}" color="${color}" size="${size}" ${disabled ? 'disabled' : ''}>${label}</DsButton>`,
	imports: ["import { DsButton } from '@drivenets/design-system'"],
	id: 'ds-button-v3',
